  ...
  return (
    <div className="space-y-4">
      <header>
        ...
        <p className="mt-1 text-sm text-gray-600">View and override appointment status across the platform.</p>
      </header>
          ...
          >
            <div>
              <div className="flex items-center gap-2">
                ...
                {appointment.booking_type === 'walkin' && (
                  <span className="rounded-full bg-stone-900 px-2 py-0.5 text-xs font-bold text-white">
                    Walk-In
                  </span>
                )}
              </div>
              <div className="text-sm text-gray-600">{appointment.shop_name} - {appointment.barber_name}</div>
              <div className="text-sm text-gray-500">
                {toDateValue(appointment.appointment_date)} at {String(appointment.appointment_time).slice(0, 5)} - {appointment.service_name}
              </div>
            </div>
            <select

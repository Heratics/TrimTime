...
export default function ShopProfile() {
  ...
  return (
    <div>
      <div className="h-52 bg-gradient-to-br from-amber-100 to-stone-300 sm:h-72">
      ...
      <div className="mx-auto max-w-6xl px-4 pb-12">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          <section className="space-y-3 lg:col-span-2">
            <h2 className="text-xl font-black">{t('profile_barbers')}</h2>
            {shop.barbers.map(barber => <BarberCard key={barber.id} barber={barber} />)}
            {!shop.barbers.length && <Empty text={t('profile_no_barbers')} />}
          </section>
          <section>
            <h2 className="text-xl font-black">{t('profile_hours')}</h2>
            <div className="mt-3 rounded-2xl border bg-white p-4">
              {shop.hours.map(hour => (
                <div key={hour.id} className="flex justify-between border-b py-2 text-sm last:border-0">
                  <span>{days[hour.day_of_week]}</span>
                  <span>{hour.is_closed ? t('profile_closed_day') : `${hour.open_time.slice(0, 5)} - ${hour.close_time.slice(0, 5)}`}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="mt-6">
          <h2 className="text-xl font-black">{t('profile_services')}</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {shop.services.map(service => <ServiceCard key={service.id} service={service} />)}
          </div>
          {!shop.services.length && <Empty text={t('profile_no_services')} />}
        </section>

        {shop.products?.length > 0 && (
          <section className="mt-6">
            <h2 className="text-xl font-black">{t('profile_products')}</h2>
              ...
              {shop.products.map(product => (
                <div key={product.id} className="rounded-2xl border bg-white shadow-sm overflow-hidden">
                  {product.image_url
                    ...
                    : <div className="h-44 bg-stone-100 flex items-center justify-center text-stone-400 text-sm">{t('profile_no_image')}</div>
                  }
                  <div className="p-4">
...
function Logo({ shop }) {
  return shop.logo_url
    ? <img src={shop.logo_url} alt="" className="h-20 w-20 rounded-2xl border bg-white object-cover" />
    : <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-stone-900 text-2xl font-black text-white">{shop.name.slice(0, 1)}</div>
}

function Empty({ text }) {
  return <div className="rounded-xl border border-dashed bg-white p-4 text-sm text-stone-500">{text}</div>
}
